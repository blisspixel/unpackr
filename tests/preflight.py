...
def analyze_test_folder():
    ...
    for folder in sorted(folders):
        ...
        rar_files = [
            ...
            if f.suffix.lower() in [".rar"] or (f.suffix.startswith(".r") and f.suffix[2:].isdigit())
        ]
    ...
    print(
        f"  Videos in root: {Fore.GREEN}{len([f for f in files if f.suffix.lower() in ['.mp4', '.mov']])}{Style.RESET_ALL}"
    )
