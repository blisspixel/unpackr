...
def test_path_cleaning(runner: ComprehensiveRunner):
    ...
    # Basic cases
    runner.test("Clean path: no quotes", clean_path("C:\\test") == "C:\\test")
    runner.test("Clean path: double quotes", clean_path('"C:\\test"') == "C:\\test")
    runner.test("Clean path: single quotes", clean_path("'C:\\test'") == "C:\\test")

    # Edge cases
    runner.test("Clean path: whitespace", clean_path("  C:\\test  ") == "C:\\test")
    runner.test("Clean path: quotes + whitespace", clean_path('  "C:\\test"  ') == "C:\\test")
    runner.test("Clean path: empty string", clean_path("") == "")
    runner.test("Clean path: only quotes", clean_path('""') == "")
    runner.test("Clean path: mixed quotes (invalid)", clean_path("\"C:\\test'") == "\"C:\\test'")
    runner.test("Clean path: spaces in path", clean_path('"C:\\Program Files\\test"') == "C:\\Program Files\\test")
...
def test_work_plan(runner: ComprehensiveRunner):
    ...
    # Test empty plan
    runner.test("WorkPlan: Empty initialization", len(plan.video_folders) == 0 and plan.total_videos == 0)

    # Add video folder
    test_path = Path("G:/test/video1")
    plan.add_video_folder(test_path, videos=2, rars=1, par2s=3)
    runner.test("WorkPlan: Add video folder - folder count", len(plan.video_folders) == 1)
    runner.test("WorkPlan: Add video folder - video count", plan.total_videos == 2)
    runner.test("WorkPlan: Add video folder - RAR count", plan.total_rars == 1)
    runner.test("WorkPlan: Add video folder - PAR2 count", plan.total_par2s == 3)

    # Add content folder
    content_path = Path("G:/test/music")
    plan.add_content_folder(content_path)
    runner.test("WorkPlan: Add content folder", len(plan.content_folders) == 1)

    # Add loose video
    video_path = Path("G:/test/video.mp4")
    plan.add_loose_video(video_path)
    runner.test("WorkPlan: Add loose video - count increment", len(plan.loose_videos) == 1 and plan.total_videos == 3)

    # Time estimate calculation
    estimate = plan.calculate_time_estimate()
    expected = (3 * 10) + (1 * 3) + (3 * 5) + (1 * 2)  # videos + rars + par2s + folders
    runner.test("WorkPlan: Time estimate calculation", estimate == expected, f"Expected {expected}, got {estimate}")
...
def test_config_edge_cases(runner: ComprehensiveRunner):
    ...
    runner.test("Config: Contains .mp4", ".mp4" in extensions)
    runner.test("Config: Contains .avi", ".avi" in extensions)
    runner.test("Config: Contains .mkv", ".mkv" in extensions)
...
def test_file_handler_boundary_cases(runner: ComprehensiveRunner):
    ...
    runner.test("FileHandler: find_video_files returns list", isinstance(empty_result, list))

    # Test removable file detection
    runner.test("FileHandler: .nfo is removable", ".nfo" in config.removable_extensions)
    runner.test("FileHandler: .sfv is removable", ".sfv" in config.removable_extensions)
    runner.test("FileHandler: .txt is removable", ".txt" in config.removable_extensions)
...
def test_video_processor_edge_cases(runner: ComprehensiveRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        runner.test("VideoProcessor: Detect 'sample' in filename", processor.is_sample_file(sample1))
        runner.test("VideoProcessor: Detect 'SAMPLE' (case insensitive)", processor.is_sample_file(sample2))
        runner.test("VideoProcessor: Regular file not sample", not processor.is_sample_file(regular))
...
def test_system_check_robustness(runner: ComprehensiveRunner):
    ...
    runner.test("SystemCheck: Returns dict", isinstance(results, dict))
    runner.test("SystemCheck: Has 7z key", "7z" in results)
    runner.test("SystemCheck: Has par2 key", "par2" in results)
    runner.test("SystemCheck: Has ffmpeg key", "ffmpeg" in results)
    runner.test("SystemCheck: Boolean values", all(isinstance(v, bool) for v in results.values()))
...
def test_statistics_accuracy(runner: ComprehensiveRunner):
    ...
    # Initial state
    runner.test("Stats: Initial folders_processed is 0", app.stats["folders_processed"] == 0)
    runner.test("Stats: Initial videos_moved is 0", app.stats["videos_moved"] == 0)
    runner.test("Stats: Initial folders_deleted is 0", app.stats["folders_deleted"] == 0)
    runner.test("Stats: Initial rars_extracted is 0", app.stats["rars_extracted"] == 0)
    runner.test("Stats: Initial par2s_repaired is 0", app.stats["par2s_repaired"] == 0)
    runner.test("Stats: Initial videos_failed is 0", app.stats["videos_failed"] == 0)
...
def test_glob_pattern_correctness(runner: ComprehensiveRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        runner.test("Glob: Finds 3 video files", len(videos) == 3)

        # Test RAR detection
        rars = list(tmp_path.glob("*.rar")) + list(tmp_path.glob("*.r[0-9][0-9]"))
        runner.test("Glob: Finds 3 RAR files (rar + r00 + r01)", len(rars) == 3)

        # Test PAR2 detection
        par2s = list(tmp_path.glob("*.par2"))
        runner.test("Glob: Finds 1 PAR2 file", len(par2s) == 1)
...
def test_error_handling(runner: ComprehensiveRunner):
    ...
    runner.test("App: Handles missing work plan gracefully", app.work_plan is None)

    # Test with empty source directory
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
        plan = app.scan_and_plan(tmp_path)
        runner.test("App: Scan empty directory creates plan", plan is not None)
        runner.test("App: Scan empty directory - zero folders", len(plan.video_folders) == 0)
...
def test_folder_classification(runner: ComprehensiveRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        runner.test(
            "Classification: Video folder detected",
            len([f for f in plan.video_folders if f["path"].name == "video_release"]) == 1,
        )
        runner.test(
            "Classification: RAR folder detected as video",
            len([f for f in plan.video_folders if f["path"].name == "rar_release"]) == 1,
        )
        runner.test(
            "Classification: Music folder detected as content",
...
def test_time_estimate_bounds(runner: ComprehensiveRunner):
    ...
    runner.test("Time estimate: Zero items = 0 seconds", estimate1 == 0)
    ...
    runner.test(
        "Time estimate: Single items calculation", estimate2 == expected2, f"Expected {expected2}, got {estimate2}"
    )
    ...
    runner.test(
        "Time estimate: Large numbers (100 folders)", estimate3 == expected3, f"Expected {expected3}, got {estimate3}"
    )
...
def test_path_edge_cases(runner: ComprehensiveRunner):
    ...
    runner.test("Path: Windows backslash preserved", "C:\\test" in clean_path("C:\\test"))
    runner.test("Path: UNC path support", clean_path('"\\\\server\\share"') == "\\\\server\\share")
    runner.test("Path: Drive letter only", clean_path('"C:"') == "C:")

    # Special characters
    runner.test("Path: Spaces preserved", "Program Files" in clean_path('"C:\\Program Files"'))
    runner.test("Path: Parentheses preserved", "(x86)" in clean_path('"C:\\Program Files (x86)"'))
