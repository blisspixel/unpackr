...
def test_state_validation(runner: DefensiveTestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        runner.test("StateValidation: Check file accessible", StateValidator.check_file_accessible(test_file))

        # Directory writable
        runner.test("StateValidation: Check directory writable", StateValidator.check_dir_writable(tmp_path))

        # Disk space check
        runner.test("StateValidation: Check disk space", StateValidator.check_disk_space(tmp_path, required_mb=1))
