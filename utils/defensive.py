...
class InputValidator:
    ...
    @staticmethod
    def validate_string(
        value: Any, min_length: int = 0, max_length: int = 10000, allow_empty: bool = True, allow_none: bool = False
    ) -> Optional[str]:
        """
        ...
        # Remove null bytes
        validated_value = value.replace("\x00", "")
        ...
        if len(validated_value) < min_length:
            raise ValidationError(f"String too short: {len(validated_value)} < {min_length}")

        if len(validated_value) > max_length:
            raise ValidationError(f"String too long: {len(validated_value)} > {max_length}")

        return validated_value

    @staticmethod
    def validate_int(
        value: Any, min_val: Optional[int] = None, max_val: Optional[int] = None, allow_none: bool = False
    ) -> Optional[int]:
        """
        ...
        if min_val is not None and validated_value < min_val:
            raise ValidationError(f"Value too small: {validated_value} < {min_val}")

        if max_val is not None and validated_value > max_val:
            raise ValidationError(f"Value too large: {validated_value} > {max_val}")
...
class ErrorRecovery:
    ...
    @staticmethod
    def safe_move(
        src: Path, dst: Path, max_attempts: int = 3, verify_integrity: bool = True, atomic: bool = True
    ) -> bool:
        """
